...
- **节点归档 vs 删除（G2）**：**硬删除**（级联删除子树）；**无 archived 状态字段**（状态最小集，G2 决策）；删除前 Service 层调 M04/M06/M07 `delete_by_node_id`（R-X2 防 CASCADE 绕过 activity_log）。
...
| **多表事务** | ✅ 三处事务（G5）| ① 拖拽同级 sort_order 批量更新（`with db.begin()` 包 N 条 UPDATE）；② 跨父 move 子树 path 更新（REPLACE path SQL + depth 更新）；③ 删除节点时先调下游 Service.delete_by_node_id（M04/M06/M07）再 DELETE nodes（R-X2） |
...
| **Service** | `api/services/node_service.py` | 业务规则 / path 字段计算 / 拖拽排序事务 / 写 activity_log<br>**对外契约**：`batch_create_in_transaction(db, project_id, nodes_data)` 供 M11/M17 调用 |
...
- **删除节点（R-X2）**：`NodeService.delete_node(node_id, project_id)` 执行顺序：
  1. 调 `M04.DimensionService.delete_by_node_id(db, node_id, project_id)` — 写 M04 activity_log
  2. 调 `M06.CompetitorService.delete_by_node_id(db, node_id, project_id)` — 写 M06 activity_log
  3. 调 `M07.IssueService.delete_by_node_id(db, node_id, project_id)` — 写 M07 activity_log
...
| `delete` | `node` | `<node_id>` | `{project_id, name, had_children: bool, subtree_count: int}` |
| `reorder` | `node` | `<parent_id 或 "root">` | `{project_id, updated_count: int, items: [{node_id, old_order, new_order}]}` |
| `move` | `node` | `<node_id>` | `{project_id, old_parent_id, new_parent_id, old_path, new_path, subtree_count, elapsed_ms}` |
