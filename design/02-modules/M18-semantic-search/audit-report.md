...
## 7.8 fix v4.2 落地清单（verify v4.1 → CY 决策 R2=A → fix v4.2）

verify v4.1 抓出 fix v4.1 是"主链修了 60%，旁链全漏"——R5'=B 拆分的连锁影响在 §12D 子模板 / §14 业务路径 / §15 cleanup cron / 辅助表 schema / baseline-patch / mock provider 默认值全没同步。fix v4.2 闭环：

**8 项必修**：
- B1 §12D 子模板 5 处同步：四形态字段位次表 ② / scan_pending 形参 / EmbedSinglePayload 示例 / 字段② 标题与核心字段 + 照抄要点全升 7 字段三段
- B2 §14 model-upgrade 端点描述 4 处升三段：扫表 WHERE / PK 描述 / 查询 WHERE / 空间代价
- B3 §15 model_version cleanup cron 谓词升三段
- B4 §14 末尾"未来 embedding 模块照抄要点"升三段
- B5 EmbeddingTask + EmbeddingFailure schema 加 provider + model_name 字段（影响 backfill recovery cron 链路：recovery 时 enqueue payload 三段填充能从 task 表取）
- O1 baseline-patch-m18.md §3.5 EmbedSinglePayload 加 model_name
- O5 backfill recovery 函数 enqueue 调用加 task.provider/model_name/model_version 三段
- O7 §14 model-upgrade env 设置说明加 EMBEDDING_MODEL_NAME（用户操作不漏 env）

**R2=A 决策（mock provider 校验，CY 拍板）**：
- §11 startup sanity check 加 mock provider + 非 `mock-*` 前缀 → ConfigError
- §11 env 表 EMBEDDING_MODEL_NAME 注释加约束说明
- tests.md 新增 tc_M18_provider_switch_02_mock_prefix 测试用例

**应修小项**：
- B6 tests.md 残留 `current_model_version` 单变量（line 35/293-300）改 7 字段元组 / current_triple
- B7 tests.md:77 `WHERE model_version=current` → 三段过滤
- B8 audit-report §7.5 fix v4 段落 EMBEDDING_MODEL_VERSION 误用记录加 fix v4.2 修订脚注（见下方）
- R3 §6 行表头与 §15 命名一致：本期 ack 现状（§6 表头是"分层 Cron"通用类目，§15 是"任务矩阵"细分；命名不冲突）
- R4 R3-4 改回成本块描述维度：本期 ack 现状（5 步够清楚）

**status 推进路径**：fix v4.2 → 派 verify v4.2 → 通过则 status=draft → accepted

---

### B8 修订脚注（fix v4 段落 EMBEDDING_MODEL_VERSION 用作 model 名误用记录）

§7.5 fix v4 段落里 `golden_05 env 名 DEFAULT_EMBEDDING_MODEL=text-embedding-4 → EMBEDDING_MODEL_VERSION=text-embedding-3-large` 是 fix v4 当时的过渡描述（彼时 EMBEDDING_MODEL_VERSION 字段过载存模型名）；fix v4.1 R5'=B 拆分后该用法已废弃，正确做法见 §7.7 / §7.8——`EMBEDDING_MODEL_NAME` 存 provider-level 模型名，`EMBEDDING_MODEL_VERSION` 仅存 product-level 业务版本号（如 `v1`）。

---

## 7.9 fix v4.3 落地清单（verify v4.2 → CY 全 ack → fix v4.3，最终闭环）

verify v4.2 抓出 4 必修（F1-F4）+ 2 旧 regression（V2/V3）+ V1 死字段嫌疑 + V4 编号倒序。CY 全按 reviewer 建议 ack。fix v4.3 闭环：

**4 必修（旁链/历史残留扫干净）**：
- F1 §4 行级状态机叙述 (model_name, model_version) 二元组 → (provider, model_name, model_version) 三元组（00-design.md:460-463）
- F2 §11 三层幂等表 content_hash 兜底 key 升 7 字段 (project_id + modality + provider + model_name + model_version + ...) 对齐 §3 真 schema + §5 R3-3（00-design.md:808）
- F3 §12C/D 对比叙述"必须 model_version 回填路径" → "必须 (provider, model_name, model_version) 三段回填路径"（00-design.md:828）
- F4 tests.md tc_M18_error_07 `EMBEDDING_MODEL_VERSION=text-embedding-foo` → `EMBEDDING_MODEL_NAME=text-embedding-foo`（与 audit-report B8 修订脚注对齐，消除"刚说废弃就出现"的自打脸）

**2 旧 regression（v3/v4 历史，accepted 前必修）**：
- V2 backfill recovery enqueue worker entry name `"embed_text"` → `"embed_single"`（与 §6 worker / §12D scan_pending payload 命名对齐，避免 arq 找不到 task handler 的硬错）
- V3 backfill recovery docstring 删 fix v4 误引用的 `EmbeddingTask.idempotency_key` 字段（该字段在 §3 schema 不存在），改用 §11 三层幂等表 worker 内 content_hash 兜底真实路径

**V1 死字段备注**：§15 monitor cron 行加备注，说明 EmbeddingFailure provider/model_name 字段本期不分桶 / 留作 Phase 2 监控扩展用（避免 simplify checklist 当死字段砍）

**V4 测试编号顺序修**：tests.md 把 tc_provider_switch_02_mock_prefix 移到 tc_provider_switch_01_mock 后面

**status 推进**：fix v4.3 完成后 grep 复检关键词全清 → status=accepted ✅
