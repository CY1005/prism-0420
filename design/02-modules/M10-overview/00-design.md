...
> **M10 sprint R1 P1-03 立修 disambiguation（2026-05-08）**：D-1 字面禁递归仅
> 约束 `_compute_completion_rates` rate 算法（深层树多次累加 rate 是 hot path）；
> `_build_tree` 树结构组装的 `_sort` 函数仍用递归（CY 场景 depth < 20 可接受 /
> 升级条件 = 实际场景 depth > 20 时改迭代 stack）。两者作用域不同：rate 算法是
> 单一真相源严守迭代；tree 排序是组装期 housekeeping，递归读法更直观。
...
> **M10 sprint 关闸 disambiguation（2026-05-08）**：上表第三行 `/completion` endpoint **走 M04 dimension_router 的 /completion 唯一注册**（避免 FastAPI 双注册路径冲突）。M10 router 仅实装 2 endpoints（overview / overview/stats）；NodeCompletionResponse schema 保留作未来 M04 委托可用。M10 service.get_node_completion 是 folder 均值算法的内部接口（保证单一真相源），不暴露 HTTP。
