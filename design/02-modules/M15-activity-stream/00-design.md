...
### Disambiguation（M15 sprint 子片 5 关闸 / 2026-05-08 实装漂移回写）

> R1 P2-1 / P2-2 punt 处理：以下三处 design §3 字面与实装存在 disambiguation，子片 0
> prep + 子片 1+3 落地，本节统一回写说明。

1. **Mapped[ActionType] / Mapped[TargetType] 字面 vs 实装 Mapped[str]**：design §3
   ActivityLog SQLAlchemy class block line 317-319 字面 `Mapped[ActionType]` /
   `Mapped[TargetType]` 引用 schema 层 Enum；实装 (`api/models/activity_log.py`) 改为
   `Mapped[str]` + `String(50)` + `CheckConstraint` 字面 IN 列表 — M02-M14 11 模块
   同款"三重防护"范式（防 model 反向依赖 schema layer 引循环 import）。
2. **metadata 列 Python 属性名重映射**：design §3 line 322 字面 `metadata: Mapped[dict]`；
   实装 Python 属性名 `event_metadata` + SQL column 名 `"metadata"`（SA Base.metadata
   保留属性冲突 disambiguation）。Service `_to_item` 把 `event_metadata` 重映射回
   schema 字段 "metadata"（与 design §7 字面 + write_event stub 字面一致 / 对外契约
   不变）。R2 P1-1 立修补 e2e 字面映射验证。
3. **list_for_team DAO 签名含 user_id 参数 vs 实装无**：design §3 line 396 字面包含
   `user_id: PyUUID` 参数；实装 DAO（`api/dao/activity_stream_dao.py:list_for_team`）
   不含 — 因 L1 require_team_access(member) 由 Router 层校验（M20 sprint 接通时注入），
   DAO 层仅读 activity_logs 不需 user_id。本 sprint Router 不暴露此 endpoint（M20
   sprint own / design §7 endpoint 表字面 1 endpoint）。
