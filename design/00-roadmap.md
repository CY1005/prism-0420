...
status: living-doc
owner: CY
last_updated: 2026-05-09（M19 sprint 完成: 8 commit + 纯只读导出 + ActionType+1 "exported" 过去式立规对齐 + R1=3 subagent 并行 + R2=1 合并 Opus 第十七数据点 + bypass log #2 配套继续 ✅ + 元教训 #19 R1 局部立修触发 tests.md 第三方文件漂移 + sink 立规候选 3 项 + cross-sprint punt 池新增 4 项）
current_phase: Phase 2.1 进行中（M01-M08+M10-M19 完成；M09 superseded by M18 不实装；下一站 M20 团队最后做）
---
...
Phase 2.1 业务模块  ⏳ ███████████████████  95%  ← M01-M08+M10-M19 完成；下一步 M20 团队（最后一个 own / M09 superseded by M18）
Phase 2.2 前端继承  ⏳ ░░░░░░░░░░░░░░░░   0%
Phase 2.3 集成验证  ⏳ ░░░░░░░░░░░░░░░░   0%
Phase 3 数据对照   ⏳ ░░░░░░░░░░░░░░░░   0%
```
...
- [x] M18 语义搜索（embedding §12D）✅ 2026-05-09（**§12D 子模板首次实战 + pgvector ARRAY 占位三层降级范式**；9 commit 90f7672 启动期 / 6c27898 子片 0 prep EmbeddingProvider mini-sprint+41 tests / fbea749 子片 1 4 表 model+alembic+pgvector ARRAY 占位+54 model tests / c7aca1f 子片 2 5 DAO 含规则 4 豁免+45 unit / f2da4e2 子片 3 Service+Schema+12 ErrorCode+SilentFailure+Queue+Cron+Provider stub+92 tests / 76c6d9b R1 立修 16 P1（3 subagent 并行）/ 68e981e 子片 4 Router 1 search+3 admin+42 e2e 元教训 18 类全覆盖 / 92e09d3 R2 立修 7+1 PUNT（1 合并 Opus）/ 子片 5 关闸；1213→1480 PASS (+267) / R13-1 124→136 / L12+L13+R14 全过；闸门 2.5 第十三次 B 0；R1+R2 第十六数据点稳定；bypass log #2 配套验收最终 ✅；元贡献 7 项：§12D 首战零摩擦 + R-X4 pgvector 占位三层范式 + EmbeddingProvider 仿 ADR-001 §4.1 + SilentFailure 占位降级 + R1+R2 16 数据点 + R10-2 例外第二实例 + 元教训 actionable 18 类完整实证；sink 立规候选 4 项：EndpointRequest 不继承 TaskPayload + pgvector 占位三层 + metadata _stub:True + 测试反模式；cross-sprint punt 池新增 5 项 #20-#24）
- [x] M19 导入/导出 ✅ 2026-05-09（**纯只读导出 / pilot=false / complexity=low / R1+R2 第十七数据点**；8 commit 3ad8efa 启动期 + 子片 0 合并到启动期 + facce18 子片 1 ActionType+1 "exported" 过去式立规对齐 + 子片 2 合并到子片 3 + 83cf44a 子片 3 ExportService+Schema+3 ErrorCode+13 service tests + c98e563 R1 立修 7 P1（3 subagent 并行 / R14 过去式 + EXPORT_EMPTY_CONTENT 422 + EXPORT_NODE_LIMIT_EXCEEDED 业务码 + ValidationError 422 范式对齐 + include 全 False schema 阻断 + dict.fromkeys 保序去重）+ 947f0e2 子片 4 Router 2 endpoints+12 e2e 元教训 18 类 + 257ab92 R2 立修 3 P1（1 合并 Opus / tests.md ↔ design ↔ exceptions 三方 status code 同步 + G2 3 node 顺序 e2e + E5 去重 e2e）+ 子片 5 关闸；1480→1512 PASS (+32) / R13-1 136→139 / L12+L13+R14 全过；闸门 2.5 第十四次 B 0；R1+R2 第十七数据点稳定；bypass log #2 配套继续 ✅；元教训 #19：R1 局部立修触发 tests.md 第三方文件漂移；sink 立规候选 3 项 SR-M19-1/2/3：tests.md 同步 checkbox + design accepted vs 立规精神对齐 + filename sanitize 输入端 vs 输出端分门别类；cross-sprint punt 池新增 4 项 #25-#28）
...
| 2026-05-09 | M18 sprint 完成（**§12D embedding 持久化首战 + pgvector ARRAY 占位三层降级范式 + R1+R2 第十六数据点 + bypass log #2 配套验收最终 ✅**；9 commit 90f7672 启动期 + 6c27898 子片 0 prep + fbea749 子片 1 + c7aca1f 子片 2 + f2da4e2 子片 3 + 76c6d9b R1 立修 + 68e981e 子片 4 + 92e09d3 R2 立修 + 子片 5 关闸；1213→1480 PASS (+267) / R13-1 124→136 / L12+L13+R14 全过；闸门 2.5 第十三次 B 0；元贡献 7 项 + sink 立规候选 4 项 + cross-sprint punt #20-#24）；Phase 2.1 85→90%；下一站 M19 导入/导出 | CY + AI |
| 2026-05-09 | M19 sprint 完成（**纯只读导出 / pilot=false / complexity=low / R1+R2 第十七数据点 / bypass log #2 配套继续 ✅**；8 commit 3ad8efa 启动期 + 子片 0 合并到启动期 + facce18 子片 1 ActionType+1 "exported" 过去式立规对齐 + 子片 2 合并到子片 3 + 83cf44a 子片 3 ExportService+Schema+3 ErrorCode+13 service tests + c98e563 R1 立修 7 P1（3 subagent 并行）+ 947f0e2 子片 4 Router 2 endpoints+12 e2e 元教训 18 类 + 257ab92 R2 立修 3 P1（1 合并 Opus）+ 子片 5 关闸；1480→1512 PASS (+32) / R13-1 136→139 / L12+L13+R14 全过；闸门 2.5 第十四次 B 0；元教训 #19：R1 局部立修触发 tests.md 第三方文件漂移；sink 立规候选 3 项 SR-M19-1/2/3：tests.md 同步 checkbox + design accepted vs 立规精神对齐 + filename sanitize 输入端 vs 输出端分门别类；cross-sprint punt #25-#28）；Phase 2.1 90→95%；下一站 M20 团队（最后 own） | CY + AI |
| _（未来变更追加在这里）_ | | |
