...
### 教训 1：立规失误模式分 4 类（2026-05-07 反向 audit 升级，原"立规分两层"扩展）

> **立规前先识别失误模式所属类型，对应不同处理**

立规失误不只"结构性 vs 工程具体"2 类。本会话实证发现 **4 类立规失误模式**：

| 失误类 | 定义 | 实例 | 处理 |
|---|----|----|----|
| **类 1 工程具体凭印象** | 立规依赖未实证的工程具体机制（如框架行为 / 库 API / 分层依赖方向）| 路径 X+ 凭 OpenAPI 是 router 还是 design 生成立子选项 | **必先实证**（≥1 次模块 sprint 真实写代码 + 跑测试后归纳）|
| **类 2 漏元维度** | 规则形式上完整但漏一个判断维度（如 horizontal vs module-specific）| §7 1.0 三选一漏判断前置 | **元层级追问**（CY"为什么 X 不应该 Y"触发审视维度清单）|
| **类 3 语义边界没分清** | 规则覆盖语义不同的多类对象但一刀切处理 | R3-6 1.0 没分启动期硬性 / 测试兜底 / 业务字典 3 子项 | **语义分类**（立规前问"这条规则覆盖的对象是同一性质吗"）|
| **类 4 凭 sense 没立规** | 团队/个人凭直觉做对了但没沉淀成规则，未来 sense 失灵时漂移 | 横切 helper 边界（M01 sprint 凭直觉放对，M02 sprint 险些失灵）| **立规防御未来**（即使 0 存量违反也立规——元教训 3）|

**应用**：立规前自审"这是哪类立规？" → 对应处理。**违反 [`feedback_external_behavior_lookup`](memory)** 主要是类 1。其他 3 类有自己的对应记忆/方法论触发。

原"结构性约束可前置 / 工程具体子选项必先实证"是类 1 vs 非类 1 的处理差异——升级版含 4 类后保留这个分层（结构性 = 非类 1，可前置 / 工程具体 = 类 1，必实证）。
...
### 教训 4：L1/L2/L3 design 维度 + scaffold 维度（正交）（2026-05-07 反向 audit 升级）
> **设计原则按"应定时机"分 design 时间维度 3 层 + scaffold 工程实施维度（与 design 维度正交）**

#### Design 时间维度（L1/L2/L3）
...
#### Scaffold 维度（与 design 维度正交，2026-05-07 反向 audit 补全）

| 层 | 实施时机 | 内容 |
|---|----|----|
| **scaffold 基础设施** | Phase 2.0 工程基线 | 横切 helper 实装（api/auth/ / api/errors/ / api/services/ 横切 service / api/models/base / 等）|
| **scaffold ↔ design reconcile** | scaffold 落地时同步 | scaffold 简化决策必含 S2 注释 4 字段（决策内容 / 简化理由 / 何时扩齐 / 触发回写）|
| **scaffold 应用 L1 规则** | L1 规则立后回扫 | 修复存量（T12 案例：10 horizontal helper docstring 加 horizontal+owner 4 字段）|

**正交说明**：scaffold 不属于 L1/L2/L3 design 维度任一层——它是工程实施基础，与 design 三层并列。L1 规则立后需要回扫 scaffold 是否符合规则（T12 修复存量），但 scaffold 自身的演进不属于 design 立规节奏。

L1 漏立晚补的代价：模块 sprint 启动 reconcile pass 必须返工补 L1 规则 + 回扫 scaffold 修复存量。本补丁 1.0 → 1.1 修订都是 L1 晚补的实证。
...
## 15. 反向 audit 修订记录（2026-05-07 后续）

CY 命令：用本文件立的迭代修复流程（A→B→C→D→E→F）反向 audit 本会话所有修改和选择。发现 4 个真问题 N1/N3/N5/N6。

### N1 元教训 1 升级 4 类立规失误模式

**发现**：原元教训 1 只覆盖"结构性 vs 工程具体"2 类，遗漏：
- 类 2 漏元维度（§7 1.0 漏横切判断前置）
- 类 3 语义边界没分清（R3-6 1.0 一刀切没分 3 子项）
- 类 4 凭 sense 没立规（横切 helper 靠 CY sense）

**修复**：§9 教训 1 升级为 4 类立规失误模式（已落）。

### N3 元教训 4 加 scaffold 维度

**发现**：L1/L2/L3 切分覆盖 design 期 3 层，但**没覆盖 scaffold 维度**（Phase 2.0 工程基线层）。T12 修复 10 helper docstring 不属 design L1/L2/L3 任一层，属 scaffold 维度（与 design 维度正交）。

**修复**：§9 教训 4 升级为"L1/L2/L3 design 维度 + scaffold 维度（正交）"（已落）。

### N5 KB 补丁01 §7 应用建议过早归纳

**发现**：KB 补丁01 §7"其他项目用主方法论时按以下顺序立 L1 规则"是基于 prism-0420 一次实证归纳的——按元教训 1 类 1 自身的精神（凭 1 次实证立通用规则会撞盲区），跨项目推广建议也应"≥2 项目实证"才立通用。

**修复**：KB 补丁01 §7 降级为"基于 1 次实证的尝试性建议，待 ≥2 个项目实证后转通用"（待落 KB 文档）。

### N6 commit bundle warn 流程改进点

**发现**：T8 commit 31 文件 bundle warn 触发但未在对话里显式报告"同主题"。CY 自动执行命令下也应主动报。

**修复**：流程改进点（不立规，下次注意）；可考虑加入 [`feedback_pre_commit_hook`](memory) 红线"bundle warn 触发时主 agent 必显式报告 bundle 主题一致性"。

### 反向 audit 元层级反思

本次反向 audit 用本文件立的元教训扫本文件立的过程——递归一层。**发现的 4 问题里 3 个是元教训自身的盲区**（N1 元教训 1 类型不全 / N3 元教训 4 维度不全 / N5 KB 应用建议过早归纳）。这本身印证元教训 1 类 1：**单次实证立的元教训也可能凭印象**——需要持续反向 audit 来发现盲区。

**新增元教训 5**（候选，待 ≥2 项目实证）：**立规则后必反向 audit 自身——用规则自己扫规则的产生过程，发现规则的盲区**。
