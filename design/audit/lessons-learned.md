...
- **2026-05-12 reconcile 3 教训 batch**：[`lessons-2026-05-12-batch.md`](./lessons-2026-05-12-batch.md)
  - 教训 A 闸门 2.5 三栏分类仪式化失效 / 教训 B 跨 sprint baseline-patch 反向修复工序 / 教训 C reconcile 工作量必 grep 真实代码
  - 来源：`_handoff/unsunk-scan-2026-05-12/chunk_6.md F-6.1/F-6.10/F-6.11`

---

## 8. 2026-05-12 unsunk-scan 路 1 三条 P1 inline 补丁

> 来源：`_handoff/unsunk-scan-2026-05-12/chunk_6.md` F-6.1 / F-6.10 / F-6.11
> 与 §7 [`lessons-2026-05-12-batch.md`](./lessons-2026-05-12-batch.md) 同源，inline 在主索引保留三段（现象/根因/对策）摘要，方便 future-self 不跳文件读完核心。

### 8.1 闸门 2.5 三栏分类自审仪式化失效（C6-F1）

**来源**：`chunk_6.md` F-6.1（session `152bff88`，2026-05-09 M05 sprint reconcile）

**现象**：M05 sprint reconcile pass 中，Claude 把 5 个本应是机械应用既有规则的项（covering 索引范式、quality-spec 关键路径、M04 punt 到期工序等）错误列入 B 栏让 CY 拍板。CY 一句"按照步骤分析做决定，做不出决定就是分析不对"点穿——重走流程后 B 栏归零。

**根因**：reconcile 自审"真有候选吗"这一步是仪式化通过的——形式上问了，但没先 grep `design/`、`audit/`、memory 三个权威源里的既有规则就答"真权衡 ✅"。判别公式漏第 0 步："我倾向的候选是不是恰好就是既有规则机械应用的结果？"——是 = A 栏，不是才 = B 栏。

**对策**：三栏自审前**必先做 5 步分层第 1-2 步**。具体操作：(1) 识别问题层 → (2) grep design/audit/memory 既有规则；命中 → 归 A 栏（机械执行）或 C 栏（用户已自决无需二次拍）；grep miss 才进入 L2 候选枚举流程，这时才有资格出现真 B 栏。已沉淀到 `feedback_problem_layered_analysis.md` 作为失效信号条目。

---

### 8.2 跨 sprint baseline-patch 反向修复工序（C6-F10）

**来源**：`chunk_6.md` F-6.10（session `fb1ffdc9`，2026-05-08 M15 sprint reconcile）

**现象**：M14 sprint 实装 `action_type` 用了 `"create"/"update"/"delete"/"link"/"unlink"` 裸 CRUD，违反 M02-M13 十一个数据点稳定的 `{entity}_{past_verb}` 命名规约（2026-05-06 baseline-patch 已立、CY 明确 ack）。M15 reconcile 时 Claude 第一次把此问题误报 B 栏，被 CY "原则性问题，有现有规则解决不了吗"再次点穿——与 §8.1 同根因再发。

**根因**：M14 实装时未 grep 既有命名规约 → 漂移产生；M15 reconcile 自审又未走 5 步分层第 1 步 grep → 漂移未识别为 L1 锁定违规反而当成 B 栏候选。命名规约 baseline-patch 是 CY 自己立的、11 模块严守——M14 是漂移源不是新基线，方向判反。

**对策**：建立"跨 sprint baseline-patch 反向修复"工序，已在 M15 sprint 验收（α 路线）：(1) grep 确认漂移范围（M14 service.py 5 处 + tests 7 处 + design §10 = 1 处，合 13 处）→ (2) 5 步分层定 L1 锁规（既有规约无歧义）→ (3) AI 自决批量改不出 B 栏让 CY 拍 → (4) 机械批量改 service.py → tests → M14 design §10 → M15 design §3 同步新值。识别信号：某模块的 action_type / ErrorCode / 命名范式与其他 ≥N 个已有模块不一致，且不一致来源是遗漏而非新决策。

---

### 8.3 reconcile 工作量必 grep 真实代码（C6-F11）

**来源**：`chunk_6.md` F-6.11（session `69105db2`，2026-05-08 M16 sprint reconcile）

**现象**：M16 冷启动 prompt 估算"M03-M08 service ~14 处裸 CRUD action_type 需反向回写"，reconcile 阶段实际 grep 后是 ~31 处（含 M02 project_service 2 处 + M11 cold_start 3 处 dot→underscore 命名漂移）。工作量从估的 4-6h 实落 8-12h，再加 write_event stub + race window 复审最终升至 12-16h——**2×-3× 偏差**。

**根因**：估算依赖 design 文字描述 + 历史印象，而非真实代码 grep。Design 记录的是设计意图、不反映代码实际分布；历史印象系统性低估了"同一错误在不同模块扩散"的程度。此事件直接触发了 `feedback_decision_codefirst_validation`（2026-05-12 立规）的 PRISM 内部首例实证。

**对策**：reconcile pass 启动前，凡涉及"批量枚举修复"类任务，**必先 grep 真实代码统计命中数再报工时区间**。具体三步：(1) `grep -rn "action_type.*create\|action_type.*delete" api/` 等模式得到真实命中行数；(2) 以命中行数 × 单次改动成本估工时区间；(3) 若 grep 结果与 design 文字描述偏差 >50%，先把真实数字同步 CY 再确认 sprint 范围，不闷头按估值跑。推广到任何"预期影响 N 处"的批量修复任务——design 文字常常只列已知模块清单，grep 才能揭出顺带命中的遗漏模块。

---

### 8.4 auto-gen marker 字面自指 bug —— hint 文本嵌入 marker 字符串触发 non-greedy 正则误匹配（2026-05-12 Phase 3 v0.3→narrative 落地揭露）

**来源**：本次 session（2026-05-12 C→B→A sprint）。`scripts/phase3_data_collector.py` 重跑 7 次后 `phase3-data-baseline.md` 累积 7 段重复段。

**现象**：baseline.md 521 行，含 1 BEGIN + 7 END + 7 段 ## 维度 1 重复——但脚本明明是"if marker exists then re.sub replace"逻辑，没有 append 路径触发条件。表面看像 append 而非 replace 的 bug。

**真 root cause**：脚本 line 333 hint 文本嵌入了字面 `{END_MARK}` 字符串：

```python
f"> 手工 narrative 段写在 {END_MARK} 之后，不会被脚本覆盖。\n\n"
```

导致每次写入的 auto_block 内部含 **2 个 END_MARK 字符串**：hint 行（行中位置）+ 真正结尾（行起手）。line 342 正则 `re.sub(rf"{BEGIN_MARK}.*?{END_MARK}\n?", ...)` 用 **non-greedy** `.*?` 匹配 BEGIN→ 第一个 END。第一个 END 在 hint 行（不是真结尾），所以正则只换掉 BEGIN→hint 段，真数据 + 真 END 被孤立留下。每次重跑残留一次 → N 次跑 = N 段重复。

**对策**（方案 1+2 同时上）：
1. **hint 文本不嵌入字面 marker**：改成"自动生成区块之后"等不含 marker 字符串的描述
2. **正则加 anchor**：`{BEGIN_MARK}.*?\n{END_MARK}\n?` 强制 END_MARK 必须行起手——既治 hint 自指（行中 marker 不匹配），也防未来 narrative 区有人引用 marker

**泛化教训**：
- **marker 字符串必须唯一且只出现在结构性位置**——不在 hint / 注释 / 文档说明里出现字面值
- **non-greedy 正则在文档替换场景禁止裸用**——必须配 `\n` / 行锚定 / 独特标识
- **N 段重复 ≠ append bug**——可能是 marker 匹配位移到错误位置。grep marker 数量是诊断第一步
- **systematic-debugging Phase 1 救场**：本次没跳"读真实代码 + 数 marker 数量"两步，否则按 "append bug" 方向乱改会加 dedup 逻辑反而埋更大坑。与 §8.3 同精神：估值前必 grep，修 bug 前必读源码 + 计数

**联动**：
- `cross-sprint-punt-pool.md` 元发现 #10（同事件、不同视角的工程教训）
- `feedback_decision_codefirst_validation`（2026-05-12 立规：决策类推荐前必读 design + grep 真实代码 + hypothesis 反验）—— 本次 fix 应用了反验步骤
