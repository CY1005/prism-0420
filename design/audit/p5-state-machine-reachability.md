...
> **状态**：F-1 / F-2 / F-3 / F-4 / F-5 / F-6 / F-7 / F-9 / F-10 已 fixed（2026-05-07）；F-8 等 contracts-draft §2 反向链契约 sprint 落地。9/10 闭环。
...
| F-2 ✅ fixed | M02 | 🟡 | M02 archive 不级联子模块状态（issue 仍 open / version 仍 is_current 等）— 设计意图但未在 §4 显式声明 | 已 patch：M02 §4 加"归档不级联子模块状态"段 + 列 M07/M05/M03/M04/M06 各自子表保持原值 + 实施约束（M02 不调子模块 service / 子模块 service 自身做 status 防御） | M02-project/00-design.md:381 |
| F-3 ✅ fixed | M07 | 🟡 | `in_progress → open`（取消认领）未在三表登记，现实场景常见但 fall-through 到通用 ISSUE_TRANSITION_INVALID | 已 patch：CY 拍方案 A——mermaid 加 in_progress→open 边 + 允许表加一行（副作用 assigned_to=NULL + activity_log issue.unassigned + 权限：仅 assignee 本人或 admin） | M07-issue/00-design.md:225,237 |
...
| F-5 ✅ fixed | M17 | 🟡 | `importing → cancelled` 副作用"回滚已写入数据"未细化——若 importing 已 commit 部分批次如何级联 DELETE 没说明 | 已 patch：CY 拍单事务——M17 §4 加 importing 阶段事务粒度段（`with db.begin():` 包整个 importing；cancelled 走 SQLAlchemy ROLLBACK；无需级联 DELETE，与 R-X1 "M17 不直 INSERT" 自洽）+ 未来超大 import 触发器 = 拆子任务而非破坏单事务范式 | M17-ai-import/00-design.md:386 |
