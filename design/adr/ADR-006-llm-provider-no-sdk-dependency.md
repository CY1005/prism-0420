...
- **API key 管理范式独立**：M13 走 `ProjectSettings.ai_api_key_enc`（M02 字段）+ AES decrypt 全链路；SDK 默认 env 读，自管 wire protocol 才能控制 key 来源
...
- **M18（embedding）范式差异 = 显式决策（F9 决议 2026-05-12 / CY 拍方案 A 接受 env-only）**：OpenAIEmbeddingProvider 从 env `OPENAI_API_KEY` 取 key 是显式决策，非 drift —— embedding 是**基础设施级**功能（搜索 / RAG）/ 与 ADR-001 §4 "embedding provider 部署期固定" 自洽 / 单租户场景 OpenAI 账号通常公司级一个 key；M13 (LLM analysis) 走 `ProjectSettings.ai_api_key_enc` AES 是**业务级**范式（每 project 独立 key / admin UI 可改 / 活动日志可审计）。**范式语义不同 / 不应强制统一**。未来多租户 SaaS / per-project embedding key → 升级方案 B（加 `ProjectSettings.embedding_api_key_enc` + alembic 迁移 + admin UI / 工作量 1.5-2d / 可逆）。**F-6.14 ✅ DONE 2026-05-12**。详见 `_handoff/unsunk-scan-2026-05-12/F9-api-key-decision.md`。
...
- F-6.14（M18 OpenAI api_key 范式对齐）✅ **DONE 2026-05-12 / CY 拍方案 A env-only / F9 决议 / 详见 `_handoff/unsunk-scan-2026-05-12/F9-api-key-decision.md`**
