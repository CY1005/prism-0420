...
###### 3.5.1 prism-0420 当前 Server Action 通道实情（Phase 2.2 子片 3a 启动期 2026-05-09 备注 / 字面零修订核心 P2 设计）

§3.5 信任链字面假设 Next.js Server Action 已通过 next-auth `getServerSession` 拿到 user_id。prism-0420 子片 0 prep 删 next-auth（spec 06 §2 决策 / access 内存 + refresh httpOnly cookie）后，Server Action 当前**没有 session 来源拿 user_id**。

**当前实施**（spec 06 §3 字面）：Server Action / Server Component 走 **α-P1 链路**：
```
cookies().get('refresh_token') → POST /auth/refresh → access_token → 
   Authorization: Bearer <access> → FastAPI（走 P1 require_user / Bearer 通道）
```

**P2 通道在 prism-0420 现状下的角色**：保留作演进选项。P1 链路已足够覆盖当前业务场景（projects / dimension / node / issue / 等普通 CRUD）。Phase 2.3 评估是否启用 P2（强安全敏感操作 / 防御深度需求）/ 启用前提是先解决 user_id 来源问题（如 server-side session cache 或 JWT sub 解析）。

**字面合规对照**：
- ADR-004 §1 P1 字面"浏览器→FastAPI 所有业务路由"是用例描述，**Server Action 走 P1 不违反字面**。
- ADR-004 §1 P2 字面"Next.js Server Action → FastAPI 服务间调用"是 P2 用例，**不强制 Server Action 必须走 P2**。
- ADR-004 §3.5 信任链是 P2 启用时的安全契约 / P2 当前未启用 / §3.5 字面留作 P2 启用时的参考。

**未来若启用 P2**：先在 spec 06 §3 加 server-auth.ts 的 P2 升级方案（refresh→access→sub→HMAC + X-User-Id）/ 触发 ADR-004 §3.5 修订（更新信任链入口为 spec 06 §3 helper / next-auth getServerSession 改写）。
