...
### 闸门 4 Phase 2.2 关闸 ✅ 后发现的盲区 #2 — 数据形态迁移半完成（2026-05-12 Phase 2.3 cleanup S3+S4 揭露）

**现象**：Phase 2.2 子片 0+1+2+3a-i+3a-ii+3b+3c+4+5 全 ✅ 标记"前端继承 100% 完成"，但 Phase 2.3 cleanup S3+S4 跑 tsc + 读真实代码后发现：

1. **`workspace.tsx` 1549 行 / 33 tsc 错** — `NodeData` 期望 camelCase（`parentId` / `dimensionTypeId` / `versionLabel` / `fieldSchema`）但 API 返 snake_case（`parent_id` / `dimension_type_id` / `version_label` / `field_schema`）；运行时这些字段实际是 `undefined`
2. **`DimensionConfig` shape gap** — workspace + import + modules 期望嵌套 `{config: {...}, dimType: {key, icon, fieldSchema, description}}`，但 API `DimensionConfigResponse` 是扁平且**缺 icon / description / field_schema** 字段
3. **`Project` shape camelCase 漂移** — 7 个文件用 `.templateType` / `.hierarchyLabels` / `.versionMode`，actions 返裸 `ProjectResponse` snake_case（2026-05-12 已补 `toProject` adapter）
4. **`getNodeWithDimensions` 缺 `projectId` 参数** — 调用方多处只传 1 arg，actions 实际签名 `(nodeId, projectId)` 两 arg
5. **legacy `services/search.ts` + `GlobalSearchBar` 死栈** — 指向不存在的 `ANALYZER_BASE_URL:8001` analyzer 服务，从未在 prism-0420 后端实装

**根因**：Phase 2.2 子片 audit 模板只验"导入 prism web 代码 + 替换 auth flow + 改 actions 调用 API"——**未验数据形态迁移完整性**。Drizzle ORM（Prism 原栈）默认 camelCase 字段，SQLAlchemy + Pydantic（prism-0420 新栈）默认 snake_case。consumer 层（pages / components）字段访问从未做大规模 grep 校对。

**量化**：
- Phase 2.2 关闸时 tsc 错数未审计（CI 当时没接通）→ 漏 88 错
- Phase 2.3 cleanup S3+S4 揭露 / 清 14 错（88→74）
- 剩 74 错全是 Drizzle→FastAPI shape gap（A 类 workspace 33 / B 类 consumer 32 / C 类 legacy search 7 / + 后端契约缺口 2）
- 估算后续 sprint：workspace 真迁移 1-2 天 + consumer adapter 0.5-1 天 + search 决策 0.5 天 = **2.5-3.5 天追补**

### 立规

🔴 **以下闸门 audit 必跑"数据形态迁移完整性扫"，不只 tsc**：
- 任何"前端继承 / 后端栈切换"类 phase 关闸（含 Phase 2.2 类继承迁移 / 未来若再做 ORM 切换 / 大版本前端框架迁移）
- 关闸 audit 模板必含 step：
  1. `grep -rn "<旧栈字段名 / 旧 shape>" app/src/ --include="*.tsx" --include="*.ts"` — 应为 0
  2. consumer 层抽样 ≥3 文件，逐字段访问对照 `src/types/api.ts` codegen 类型
  3. 后端 schema 与前端期望对齐：`grep -rn "\.fieldSchema\|\.description\|<前端独有字段>" app/src/` vs 后端 schema 实际字段
  4. 跑 `pnpm exec tsc --noEmit` 必 0 错（CI build job 上限值）

🔴 **"继承"≠"运行" 红线**：
- 拷贝代码后必须**真跑 happy path**（至少 1 个核心 page 操作流程从 UI 点到 API 收到响应），不能只看 `next build` 编译通过
- 编译通过 ≠ 运行正确（snake_case 字段访问 camelCase 在 TS 编译期 OK / 运行期返 `undefined`）

🔴 **adapter 单点收敛 vs consumer 全量重写选择**：
- 业务字段在 consumer 端访问 **≥5 文件** → 在 actions 层加 `toX(raw): X` mapper（单点改），不要逐文件 rename
- consumer 端访问 < 5 → 直接 rename consumer 端，不要为单 caller 加 mapper

### STAR 素材标签
- **模式归类**：契约漂移 28% 模式的真子类——"双栈迁移半完成 / 编译绿但运行 broken"。比"删 export 后忘扫调用方"严重一档（前者编译 broken / 这次是运行 broken / 更难发现）
- **量化**：Phase 2.2 子片 9 个全 ✅ 但漏 70+ shape gap = 关闸模板覆盖率不足 ≈ 80%
- **教训**：双栈迁移类项目的关闸**必须有"运行 happy path 真跑"环节**——CI build / tsc 都是静态检查，发现不了"camelCase 字段访问 snake_case API"这种类型系统帮不上忙的运行时 bug
