...
async def test_dao_list_by_project_returns_all(
    db_session, dao, make_project, make_node, make_module_relation
):
    ...
    await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
    await make_module_relation(user=user, project=proj, source_node=n2, target_node=n3)
    rows = await dao.list_by_project(db_session, proj.id)
    assert len(rows) == 2


async def test_dao_list_by_node_bidirectional(
    db_session, dao, make_project, make_node, make_module_relation
):
    ...
    await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
    await make_module_relation(user=user, project=proj, source_node=n3, target_node=n1)
    await make_module_relation(user=user, project=proj, source_node=n2, target_node=n3)
...
async def test_dao_list_by_project_isolates_tenants(
    db_session, dao, make_project, make_node, make_module_relation
):
    ...
    await make_module_relation(user=user, project=projA, source_node=nA1, target_node=nA2)
    await make_module_relation(user=user, project=projB, source_node=nB1, target_node=nB2)
...
async def test_dao_get_by_id_in_tenant(
    db_session, dao, make_project, make_node, make_module_relation
):
    user, proj = await make_project()
    n1, n2 = await make_node(proj.id, name="A"), await make_node(proj.id, name="B")
    r = await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
...
async def test_dao_get_by_id_blocks_cross_tenant(
    ...
    _, projB = await make_project(name_suffix="-B")
    nA1, nA2 = await make_node(projA.id, name="A1"), await make_node(projA.id, name="A2")
    r = await make_module_relation(user=user, project=projA, source_node=nA1, target_node=nA2)
...
async def test_dao_update_notes(db_session, dao, make_project, make_node, make_module_relation):
    user, proj = await make_project()
    n1, n2 = await make_node(proj.id, name="A"), await make_node(proj.id, name="B")
    r = await make_module_relation(
        user=user, project=proj, source_node=n1, target_node=n2, notes="orig"
    )
...
async def test_dao_update_notes_blocks_cross_tenant(
    db_session, dao, make_project, make_node, make_module_relation
):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
...
async def test_dao_delete_by_id(db_session, dao, make_project, make_node, make_module_relation):
    user, proj = await make_project()
    n1, n2 = await make_node(proj.id, name="A"), await make_node(proj.id, name="B")
    r = await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
...
async def test_dao_delete_by_node_id_bidirectional(
    db_session, dao, make_project, make_node, make_module_relation
):
    ...
    r1 = await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
    r2 = await make_module_relation(user=user, project=proj, source_node=n3, target_node=n1)
    r3 = await make_module_relation(user=user, project=proj, source_node=n2, target_node=n3)
...
async def test_dao_delete_by_node_id_blocks_cross_tenant(
    db_session, dao, make_project, make_node, make_module_relation
):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
...
async def test_dao_list_by_node_for_delete(
    db_session, dao, make_project, make_node, make_module_relation
):
...
async def test_dao_count_by_project(db_session, dao, make_project, make_node, make_module_relation):
    user, proj = await make_project()
    n1, n2 = await make_node(proj.id, name="A"), await make_node(proj.id, name="B")
    await make_module_relation(user=user, project=proj, source_node=n1, target_node=n2)
...
async def test_dao_search_by_keyword(
    db_session, dao, make_project, make_node, make_module_relation
):
    """M09 pilot pass-through: notes ilike。"""
    user, proj = await make_project()
    n1, n2 = await make_node(proj.id, name="A"), await make_node(proj.id, name="B")
    await make_module_relation(
        user=user,
        project=proj,
        source_node=n1,
        target_node=n2,
...
async def test_dao_search_by_keyword_blocks_cross_tenant(
    db_session, dao, make_project, make_node, make_module_relation
):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    nA1, nA2 = await make_node(projA.id, name="A1"), await make_node(projA.id, name="A2")
    await make_module_relation(
        user=user, project=projA, source_node=nA1, target_node=nA2, notes="secret"
    )
