...
# M04 sprint：_make_node 也已抽到 conftest.py:make_node fixture（同款规则延伸）。
...
async def test_dao_list_by_project_orders_by_depth_then_sort_order(
    db_session, dao, make_project, make_node
):
    _, proj = await make_project()
    root = await make_node(proj.id, name="root")
    c1 = await make_node(proj.id, parent=root, name="c1", sort_order=1)
    c0 = await make_node(proj.id, parent=root, name="c0", sort_order=0)
...
async def test_dao_list_by_project_isolates_tenants(db_session, dao, make_project, make_node):
    ...
    await make_node(projB.id, name="B")
...
async def test_dao_get_by_id_returns_node_in_tenant(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    n = await make_node(proj.id, name="x")
...
async def test_dao_list_children_root_level(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    r1 = await make_node(proj.id, name="r1", sort_order=0)
    r2 = await make_node(proj.id, name="r2", sort_order=1)
    # 子节点不应出现在 root list
    await make_node(proj.id, parent=r1, name="c1")
...
async def test_dao_list_children_specific_parent(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    root = await make_node(proj.id, name="root")
    c0 = await make_node(proj.id, parent=root, name="c0", sort_order=0)
    c1 = await make_node(proj.id, parent=root, name="c1", sort_order=1)
...
async def test_dao_list_subtree_includes_self_and_descendants(
    db_session, dao, make_project, make_node
):
    _, proj = await make_project()
    root = await make_node(proj.id, name="root")
    c1 = await make_node(proj.id, parent=root, name="c1")
    gc = await make_node(proj.id, parent=c1, name="gc")
    sibling = await make_node(proj.id, name="sibling")  # 无关 root
...
async def test_dao_list_subtree_anchor_not_found_returns_empty(
    db_session, dao, make_project, make_node
):
    _, proj = await make_project()
...
async def test_dao_list_subtree_blocks_cross_tenant(db_session, dao, make_project, make_node):
    _, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    nA = await make_node(projA.id, name="A")
...
async def test_dao_max_sort_order_root_level(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    await make_node(proj.id, name="r0", sort_order=0)
    await make_node(proj.id, name="r5", sort_order=5)
    await make_node(proj.id, name="r2", sort_order=2)
    assert await dao.max_sort_order(db_session, None, proj.id) == 5


async def test_dao_max_sort_order_empty_returns_none(db_session, dao, make_project, make_node):
    _, proj = await make_project()
...
async def test_dao_bulk_update_sort_order_updates_only_specified(
    db_session, dao, make_project, make_node
):
    _, proj = await make_project()
    a = await make_node(proj.id, name="a", sort_order=0)
    b = await make_node(proj.id, name="b", sort_order=1)
    c = await make_node(proj.id, name="c", sort_order=2)
...
async def test_dao_bulk_update_sort_order_blocks_cross_parent(
    db_session, dao, make_project, make_node
):
    """重排只能在同 parent_id 范围内（即使 ID 列表跨 parent 也不更新）。"""
    _, proj = await make_project()
    root1 = await make_node(proj.id, name="root1")
    root2 = await make_node(proj.id, name="root2")
    c1 = await make_node(proj.id, parent=root1, name="c1", sort_order=0)
    c2 = await make_node(proj.id, parent=root2, name="c2", sort_order=0)
...
async def test_dao_update_paths_in_subtree_replaces_prefix_and_depth(
    db_session, dao, make_project, make_node
):
    """move_subtree 语义验证：prefix REPLACE + depth delta（G5）。"""
    _, proj = await make_project()
    # /A/, /A/B/, /A/B/C/   →  move B 到 newRoot：/A/ 不变，/A/B/ → /newRoot/B/
    A = await make_node(proj.id, name="A")  # depth=0 path="/A_id/"
    B = await make_node(proj.id, parent=A, name="B")  # depth=1
    C = await make_node(proj.id, parent=B, name="C")  # depth=2
    new_root = await make_node(proj.id, name="NR")  # depth=0
...
async def test_dao_update_paths_in_subtree_isolates_tenants(
    db_session, dao, make_project, make_node
):
    _, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    await make_node(projA.id, name="X")
    b = await make_node(projB.id, name="X")  # 同 prefix 形态
...
async def test_dao_create_persists_node(db_session, dao, make_project, make_node):
    _, proj = await make_project()
...
async def test_dao_update_fields_persists(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    n = await make_node(proj.id, name="old")
...
async def test_dao_delete_one_returns_rowcount(db_session, dao, make_project, make_node):
    _, proj = await make_project()
    n = await make_node(proj.id, name="del")
...
async def test_service_get_for_embedding_returns_name_plus_description(
...
async def test_service_get_for_embedding_returns_none_when_not_found(
    db_session, make_project, make_node
):
    _, proj = await make_project()
    svc = NodeService()
...
async def test_service_get_for_embedding_blocks_cross_tenant(db_session, make_project, make_node):
