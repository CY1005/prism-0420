...
async def test_create_dimension_returns_201(auth_client, make_user, make_dim_type):
    ...
    type_id = await make_dim_type(project_id=pid, key="g1")
...
async def test_create_dimension_disabled_type_returns_422(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-disabled@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="g1_off", enabled=False)
...
async def test_create_dimension_duplicate_returns_409(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-dup@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="g1_dup")
...
async def test_list_dimensions_returns_items_and_enabled_types(
    auth_client, make_user, make_dim_type
):
    user = await make_user(email="m04-list@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    t1 = await make_dim_type(project_id=pid, key="lst1", sort_order=1)
    await make_dim_type(project_id=pid, key="lst2", sort_order=2)
...
async def test_get_one_returns_record(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-getone@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="getone")
    await auth_client.post(
        f"/api/projects/{pid}/nodes/{nid}/dimensions",
...
async def test_update_dimension_increments_version(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-upd@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="upd")
...
async def test_update_dimension_version_conflict_returns_409(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-upd-conf@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="updc")
    await auth_client.post(
        f"/api/projects/{pid}/nodes/{nid}/dimensions",
        json={"dimension_type_id": type_id, "content": {}},
        headers=_bearer(user.id),
...
async def test_update_dimension_not_found_returns_404(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-upd-nf@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="upd_nf")
...
async def test_delete_dimension_returns_204(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-del@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    type_id = await make_dim_type(project_id=pid, key="del")
    await auth_client.post(
        f"/api/projects/{pid}/nodes/{nid}/dimensions",
        json={"dimension_type_id": type_id, "content": {}},
        headers=_bearer(user.id),
...
async def test_completion_returns_filled_and_enabled_count(auth_client, make_user, make_dim_type):
    user = await make_user(email="m04-comp@example.com")
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    t1 = await make_dim_type(project_id=pid, key="comp1")
    await make_dim_type(project_id=pid, key="comp2")
    await make_dim_type(project_id=pid, key="comp3", enabled=False)
...
async def test_create_dimension_non_member_returns_404(auth_client, make_user, make_dim_type):
    ...
    type_id = await make_dim_type(project_id=pid, key="intr")
...
async def test_update_dimension_blocks_cross_tenant_node_id(auth_client, make_user, make_dim_type):
    ...
    type_id_B = await make_dim_type(project_id=pidB, key="x")
...
async def test_list_dimensions_excludes_disabled_types(auth_client, make_user, make_dim_type):
    ...
    pid = await _create_project(auth_client, user.id)
    nid = await _create_node(auth_client, user.id, pid, name="A")
    await make_dim_type(project_id=pid, key="enabled_t")
    await make_dim_type(project_id=pid, key="disabled_t", enabled=False)
