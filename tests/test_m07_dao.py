...
async def test_dao_list_by_project_basic(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    await make_issue(user=user, project=proj, title="i1")
    await make_issue(user=user, project=proj, title="i2")
    rows = await dao.list_by_project(db_session, proj.id)
    assert len(rows) == 2


async def test_dao_list_by_project_filter_category(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    await make_issue(user=user, project=proj, category="bug")
    await make_issue(user=user, project=proj, category="tech_debt")
    rows = await dao.list_by_project(db_session, proj.id, category="bug")
    assert len(rows) == 1
    assert rows[0].category == "bug"


async def test_dao_list_by_project_filter_status(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    await make_issue(user=user, project=proj, status="open")
    await make_issue(user=user, project=proj, status="resolved")
    rows = await dao.list_by_project(db_session, proj.id, status="open")
    assert len(rows) == 1


async def test_dao_list_by_project_filter_node_id(
    db_session, dao, make_project, make_node, make_issue
):
    user, proj = await make_project()
    n1 = await make_node(proj.id, name="N1")
    n2 = await make_node(proj.id, name="N2")
    await make_issue(user=user, project=proj, node=n1)
    await make_issue(user=user, project=proj, node=n2)
    await make_issue(user=user, project=proj, node=None)  # 游离

    rows = await dao.list_by_project(db_session, proj.id, node_id=n1.id)
    assert len(rows) == 1


async def test_dao_list_by_project_filter_tag(db_session, dao, make_project, make_issue):
    """JSONB tag 查询：tags @> [tag]。"""
    user, proj = await make_project()
    await make_issue(user=user, project=proj, tags=["p0", "regression"])
    await make_issue(user=user, project=proj, tags=["p1"])
    await make_issue(user=user, project=proj, tags=[])
...
async def test_dao_list_by_project_isolates_tenants(db_session, dao, make_project, make_issue):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    await make_issue(user=user, project=projA, title="A")
    await make_issue(user=user, project=projB, title="B")

    rowsA = await dao.list_by_project(db_session, projA.id)
    assert len(rowsA) == 1


async def test_dao_list_by_project_respects_limit(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    for i in range(5):
        await make_issue(user=user, project=proj, title=f"i{i}")
...
async def test_dao_get_by_id_in_tenant(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    i = await make_issue(user=user, project=proj)
    found = await dao.get_by_id(db_session, i.id, proj.id)
    assert found is not None and found.id == i.id


async def test_dao_get_by_id_blocks_cross_tenant(db_session, dao, make_project, make_issue):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    i = await make_issue(user=user, project=projA)
...
async def test_dao_get_for_update_returns_locked_row(db_session, dao, make_project, make_issue):
...
async def test_dao_count_by_project(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    await make_issue(user=user, project=proj, status="open")
    await make_issue(user=user, project=proj, status="open")
    await make_issue(user=user, project=proj, status="resolved")
...
async def test_dao_update_changes_fields(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    i = await make_issue(user=user, project=proj, title="orig")
...
async def test_dao_update_blocks_cross_tenant(db_session, dao, make_project, make_issue):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    i = await make_issue(user=user, project=projA)
...
async def test_dao_delete_by_id(db_session, dao, make_project, make_issue):
    user, proj = await make_project()
    i = await make_issue(user=user, project=proj)
...
async def test_dao_delete_by_id_blocks_cross_tenant(db_session, dao, make_project, make_issue):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    i = await make_issue(user=user, project=projA)
...
async def test_dao_list_by_node_for_orphan(db_session, dao, make_project, make_node, make_issue):
    user, proj = await make_project()
    node = await make_node(proj.id, name="A")
    await make_issue(user=user, project=proj, node=node)
    await make_issue(user=user, project=proj, node=node)
    await make_issue(user=user, project=proj, node=None)  # 游离不应返回

    rows = await dao.list_by_node_for_orphan(db_session, node.id, proj.id)
    assert len(rows) == 2


async def test_dao_orphan_by_node_id_sets_null(
    db_session, dao, make_project, make_node, make_issue
):
    """**R-X2 orphan 关键测试**：UPDATE node_id = NULL（不是 DELETE）。"""
    user, proj = await make_project()
    node = await make_node(proj.id, name="A")
    i1 = await make_issue(user=user, project=proj, node=node)
    i2 = await make_issue(user=user, project=proj, node=node)
    other = await make_issue(user=user, project=proj, node=None)
...
async def test_dao_orphan_by_node_id_blocks_cross_tenant(
    db_session, dao, make_project, make_node, make_issue
):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(name_suffix="-B")
    nA = await make_node(projA.id, name="A")
    iA = await make_issue(user=user, project=projA, node=nA)
