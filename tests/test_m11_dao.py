...
R1-B P1-01 立修（2026-05-08）：内联 _make_task 已迁 tests/conftest.py make_cold_start_task。
"""
...
async def test_dao_list_by_project_returns_user_tasks_desc(
    db_session, dao, make_project, make_cold_start_task
):
    """created_at desc 排序：手动设置不同 created_at（PG `now()` 在同事务内返回相同值）。"""
    from datetime import UTC, datetime, timedelta

    user, proj = await make_project()
    t1 = await make_cold_start_task(project_id=proj.id, user_id=user.id)
    t2 = await make_cold_start_task(project_id=proj.id, user_id=user.id)
...
async def test_dao_list_by_project_filters_by_user(
    db_session, dao, make_project, make_cold_start_task
):
    ...
    await make_cold_start_task(project_id=projShared.id, user_id=userA.id)
    await make_cold_start_task(project_id=projShared.id, user_id=userB_user.id)
...
async def test_dao_list_by_project_isolates_tenants(
    ...
    await make_cold_start_task(project_id=projA.id, user_id=user.id)
    await make_cold_start_task(project_id=projB.id, user_id=user.id)
    rowsA = await dao.list_by_project(db_session, projA.id, user.id)
    assert len(rowsA) == 1
    assert rowsA[0].project_id == projA.id


async def test_dao_list_by_project_respects_limit(
    db_session, dao, make_project, make_cold_start_task
):
    user, proj = await make_project()
    for _ in range(5):
        await make_cold_start_task(project_id=proj.id, user_id=user.id)
...
async def test_dao_get_by_id_in_tenant(db_session, dao, make_project, make_cold_start_task):
    user, proj = await make_project()
    t = await make_cold_start_task(project_id=proj.id, user_id=user.id)
    found = await dao.get_by_id(db_session, t.id, proj.id)
    assert found is not None
    assert found.id == t.id


async def test_dao_get_by_id_cross_tenant_returns_none(
    db_session, dao, make_project, make_cold_start_task
):
    user, projA = await make_project(name_suffix="-A")
    _, projB = await make_project(owner=user, name_suffix="-B")
    t = await make_cold_start_task(project_id=projA.id, user_id=user.id)
    found = await dao.get_by_id(db_session, t.id, projB.id)
    assert found is None, "跨 project 越权 GET 必须返 None"


async def test_dao_get_by_id_unknown_returns_none(
    db_session, dao, make_project, make_cold_start_task
):
...
async def test_dao_create_persists(db_session, dao, make_project, make_cold_start_task):
    user, proj = await make_project()
    task = ColdStartTask(
        project_id=proj.id,
        user_id=user.id,
...
async def test_dao_update_status_and_counts(db_session, dao, make_project, make_cold_start_task):
    user, proj = await make_project()
    t = await make_cold_start_task(project_id=proj.id, user_id=user.id)
...
async def test_dao_update_cross_tenant_no_effect(
    db_session, dao, make_project, make_cold_start_task
):
...
async def test_dao_update_unknown_id_returns_zero(
    db_session, dao, make_project, make_cold_start_task
):
    _, proj = await make_project()
    rowcount = await dao.update(
...
async def test_dao_update_empty_fields_raises(db_session, dao, make_project, make_cold_start_task):
    user, proj = await make_project()
    t = await make_cold_start_task(project_id=proj.id, user_id=user.id)
