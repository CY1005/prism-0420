...
| **11** | **IntegrityError → 500 跨模块 3 处（M02 project create / M04-4 dimension B3 / M04-17 C7.1）** | M04 R1-A B3 + R1-C C7.1 | M05 sprint 顺修（已过） | **high** — create 路径仅预查 + DB UNIQUE 兜底，竞态暴 500 给前端；触发点 B 跨模块 | M16 启动期立通用规则（feedback 或 design §13 R13 段：service 层 INSERT 凡 UNIQUE 必 catch IntegrityError 转 *DuplicateError）|
