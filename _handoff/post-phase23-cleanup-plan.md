...
owner: CY
created: 2026-05-10
last_updated: 2026-05-10 (**Sprint 1 ✅ + Sprint 4 ✅ 含 4C.3 SR-DETACH-1 跨模块 5 模块同步立修 / 进入等 CY 状态：Sprint 2 启 server + Sprint 3 加 PAT scope**)
...
status_sprint2: 🟡 PARTIAL 2026-05-10 / Task 2.1+2.2 基础设施 ✅ + Task 2.3 spec 03/04/05/06 ✅（10 e2e PASS / 含 D 类 #3 join 实证 + L1-α detach 实证 + 4C.3 expunge fix）+ Task 2.4 简化版 ✅（5 benchmark / 100 project seed / baseline.json 落 / pytest 1638→1643）/ Task 2.3 spec 07-10（AI snapshot / import-export / search / relation graph）punt（依赖 mock provider / WS / multipart / XYFlow / 各自独立子 sprint）/ Task 2.4 完整版（1000 seed + CI 接通 + sync wrapper）punt 下次 / Task 2.5 R1+R2 punt（cost / 等 spec 07-10 完成才齐数据点）
status_sprint3: ⏸ 阻塞（GH PAT 缺 workflow scope / CY 加 scope 后 5 min 推 ci.yml / ci.yml 已确认无 e2e job 不会留红 / Sprint 3.2 e2e job 接通仍需 Sprint 2 完成）
status_sprint4: ✅ DONE 2026-05-10 / 4A 全 4 项 + 4C.1 + 4C.2 在 2026-05-09 M-CLEANUP sprint 已做完（commits 33b5759+aabde04+5c7783d）/ **4C.3 SR-DETACH-1 跨模块 5 模块同步立修 ✅ 本次完成**（CY 选 A2 / L1-α 用户路径完整性原则压制 L1-β 状态机 / M02+M03+M05+M06×2+M07 5 模块改造 / 6 detach unit tests / pytest 1629→1638 PASS / design `06-design-principles.md` 附录立 L1-α + ci-lint R16+R17 grep 候选 / cross-sprint pool 标 #6 + M02-A1 ✅ DONE）/ 4B 三项 #21+#23+#24 触发条件未到（A 类占位期残留 / 等 pgvector 真接通 + 真业务 path / 不在本 cleanup 范围）
routing_decision: Sprint 1 ✅ → Sprint 4 ✅（含 4C.3 跨模块 SR-DETACH-1）→ 进入等 CY 状态（Sprint 2 启 server / Sprint 3 加 PAT scope）
